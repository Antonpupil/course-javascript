...
function findAllPSiblings(where) {
  ...
  /* for(let i=0; i<where.children.length; i++){
    if(where.child[i].nextElementSibling && where.child[i].nextElementSibling.tagName == 'p'){
      arrChildNextP.push(where.child[i]);
    }
    return arrChildNextP;
  } */
  for (const el of where.children) {
    if (el.nextElementSibling && el.nextElementSibling.tagName === 'P') {
      arrChildNextP.push(el);
    }
  }
  return arrChildNextP;
}
